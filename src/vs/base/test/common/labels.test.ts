...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), 'Mike-AI-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), 'Untitled-1 - Mike-AI-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), 'monaco - Mike-AI-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), 'somefile.txt - monaco - Mike-AI-IDE');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), '* somefile.txt - monaco - Mike-AI-IDE');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Mike-AI-IDE', separator: { label: ' - ' } }), 'monaco - Mike-AI-IDE');
	});
